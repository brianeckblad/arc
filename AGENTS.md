...
| `write validation` (`commit check`, `set`/`commit` pre-flight) | `app/commands/request_validate.py` + `app/settings/request_schemas.py` (AUTO-GENERATED), `_commit_check`/`_commit_preflight` in `app/shell/configure.py` | `request_validate.py` or `python app/scripts/generate_request_schemas.py` | `--only 3,13` |
...
| `scm` (login/setup/profiles) | `_cmd_scm`/`_pick_profile` in `app/shell/configure.py`, `_switch_profile`/`_print_profile_list` in `app/shell/navigation.py`, shared wizard `app/auth/wizard.py`, `docs/commands/scm.md` | same | `--only 1,2,9` |
| `connect` (device SSH) | `_cmd_connect`/`_pick_folder_device` in `app/shell/sessions.py`, `docs/commands/connect.md` | same | `--only 1,2,9` |
| `setup` (guided setup) | `setup_scm`/`_run_env_wizard`/`_run_auth_configure` in `app/cli.py` (env-var wizard + keychain wizard); OS guides via `render_doc_file` in `app/docs.py` | `app/cli.py` | `--only 1,2,9` |
...
└── app/                           ← CODE ONLY
    ...
    ├── api/_auth.py               ← oauth_token (access_token) — shared SCM/SLS client-credentials grant
...
- **Shared GUI base:** both browser consoles extend `BaseGuiServer` (`app/web/gui_base.py`) and share `/assets/gui.css` + `/assets/gui.js`. `arc gui-configure` manages preferences, config.json, **credential profiles (create/switch/delete/edit + keychain vs file)**, **command aliases**, auth, branding, API sources, appearance and maintenance.
- **Unified GUI:** `feature gui-configure` is one consistent SPA — top tabs (Areas · Features · Command Structure · Built-ins · Advanced), a left sidebar of groups per section, a main pane of items, `#section/group` hash routing. Built-in visibility is edited through the same `load_command_visibility` the shell's `?`/dispatch use; file/area names come from the shared `feature_labels` layer (human-readable in GUI and CLI). (Aliases now live in the **arc** GUI, not the feature GUI.)
...
- **Write validation is universal + auto-generated.** Every staged write body is checked against the OpenAPI request schema of the endpoint it hits — route-keyed `(method, base_url, path-template)`, so it covers ALL commands (generated AND hand-written) and every future feature with no per-command code. Artifact: `app/settings/request_schemas.py` (AUTO-GENERATED by `app/scripts/generate_request_schemas.py`, in the `catalog rebuild` chain); validator: `validate_request_body_by_route` in `app/commands/request_validate.py`. Runs at three points: `set` (soft warning, still stages), `commit check` (offline schema pass + connected state re-check), and a `commit` pre-flight that prompts **proceed / fix (keep) / abandon (discard)** on problems. SCM is the final authority — its rejection is surfaced via `_format_api_error`. `id`/readOnly excluded from required; a required field counts as present if it's in the body OR the query params (folder/position).
...
- **Storage modes:** `keychain` (default, secure) vs `file` (plaintext auth.json, opt-in with warnings). Prompted in the `arc setup scm keystore` / `arc auth configure` wizard and settable in `arc gui-configure` — full parity, both via `app/config.py` (`load_config`/`save_config`) which is the single source of truth. Old config.json + keychain auto-migrate on first save.
- **The credential wizard is one shared implementation** in `app/auth/wizard.py` (`run_credential_wizard` + `select_or_create_profile` + prompt helpers), called by BOTH the `arc` launcher and the in-shell `scm setup` — so the surfaces never drift. When run without an explicit profile and profiles already exist, it shows a **pick-existing-or-create-new menu** (no `--profile` flag needed); first-time setup auto-names the profile after the account (slugified `client_id` stem) instead of `default`; explicit `--profile`/name always wins. It prompts storage mode (keychain vs file) and auto-verifies the token.
- **Setup lives in `app/cli.py`** (the launcher, moved off the shell). `arc setup scm` (bare) → `_scm_manual_and_offer` → `_run_env_wizard` = session-only **env-var** creds (paste a bearer token, or mint one from a service account); writes nothing to disk. `arc setup scm keystore` / `arc auth configure` → `run_credential_wizard` (the persistent keychain wizard). SCM API auth is automatic (client-credentials at startup); there is no interactive SCM sign-in and no userinfo/identity fetch. Token expiry is the REAL `expires_in` (omit when unknown); expired tokens purged at startup (`_init_clients`).
- **In-shell credential/profile management is the `scm` command** (`_cmd_scm` in `app/shell/configure.py`): `scm login [profile]` (switch + authenticate; picker when omitted), `scm setup [profile]` (create/edit via the shared wizard, then log in), `scm status`, `scm delete <name>`, `scm gui`. `account` / `account <name>` is an **alias of `scm login`**. Profile switch/auth is `_switch_profile` in `app/shell/navigation.py` (reload config → new `SCMClient` → reset context → `set_active_profile`), shared by both. The arc GUI (`arc gui-configure` → Profiles) does the same CRUD in the browser. The `connect` command is **device SSH** (folder-aware — no arg uses the current folder's device, prompting if several), NOT SCM auth.
