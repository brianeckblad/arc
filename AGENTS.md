...
└── app/                            ← CORE CODE ONLY (no user assets live here)
    ...
    │   ├── resource_catalog.py     ← AUTO-GENERATED catalog of uncovered NGFW list endpoints (do not hand-edit)
    │   └── generated.py            ← factory: builds ungated `show <resource>` commands from resource_catalog (100% coverage)
    └── utils/
...
#### 100% NGFW coverage policy — read endpoints are auto-exposed

**Policy: every folder-scoped NGFW config list endpoint in the pulled specs is
reachable as a `show <resource>` command — always, with no per-resource code.**

| Piece | Role |
|-------|------|
| `dev/generate_resource_catalog.py` | Reads `docs/scm-api/specs/ngfw-*.yaml`, subtracts endpoints already covered by an explicit `show` command (via doc front-matter `api:`), writes `app/commands/resource_catalog.py`. |
| `app/commands/resource_catalog.py` | AUTO-GENERATED list of uncovered resources (do not hand-edit). |
| `app/commands/generated.py` | Factory turning each catalog entry into an **ungated** (always-on) `show <resource>` command calling `SCMClient.get_config(domain, path, folder)`. Merged FIRST in `registry.py` so explicit commands win. |
| `docsupdate` | Runs `generate_resource_catalog.py` then `generate_command_docs.py`, so new pan.dev endpoints become commands + docs automatically. |
...
Generated commands are intentionally ungated (no `feature_flag`) — the full read
surface is always on. Scope today: `objects`, `security`, `network`, `identity`
(the folder-scoped config domains). When an explicit command is added for a
resource, the generator drops the generic one on the next run (explicit wins).
...
| `settings` | User-editable assets | `settings/` (banner, theme, goodbye, cli-structure, features), `app/paths.py` | Banner, colours, labels, exit messages — no code (command help text lives in docs/commands/*.md) |
