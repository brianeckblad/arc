...
- After editing any 300+ line file, run `python dev/gen_code_map.py` (the pre-commit hook at `.git/hooks/pre-commit` also auto-refreshes it when any `app/*.py` file is staged; install it once with `bash dev/install_hooks.sh`)
