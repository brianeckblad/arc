...
#### Token-efficient agent workflow — read QUICK.md first

**`QUICK.md`** (repo root) is a 150-line context map that covers 90% of tasks without reading this full file.

| Start here | Then read if needed |
|-----------|-------------------|
| `README.dev.md` — user keyword dictionary + request templates | `QUICK.md` — agent recipes and file ownership |
| `QUICK.md` — file ownership, keyword vocabulary, add-command steps, debug patterns, smoke section map | `AGENTS.md` — full spec, security rules, SCM gateway map |
| `dev/API_INDEX.md` — compact endpoint table (299 lines, all specs) | `docs/scm-api/specs/<cat>.md` — full spec for one category |
...
**Read minimally:**
- Read `QUICK.md` (not `AGENTS.md`) for routine tasks
- Read `README.dev.md` when the user asks how to phrase requests or which keywords to use
...
- `dev/API_INDEX.md` (299 lines) replaces reading any spec file for endpoint lookups

**Never hand-maintain line numbers:**
- `dev/CODE_MAP.md` is generated by `python dev/gen_code_map.py` and drift-checked by smoke section 10
- Do not write line numbers into docstrings or comments — they rot. Point to `dev/CODE_MAP.md` instead
- After editing any 300+ line file, run `python dev/gen_code_map.py` (the pre-commit hook also auto-refreshes it)
...
```
The debug error table in `QUICK.md` maps error text to the 1–2 relevant files.
...
├── README.dev.md                   ← developer keyword dictionary + token-efficient request templates
├── QUICK.md                        ← agent context map (read this first; saves tokens)
...
(consolidated endpoint listing), `guides/*.md` (every conceptual pan.dev doc
under `products/scm/docs/`), plus `index.md`, `CHANGES.md`, and a `MANIFEST.md`
that records each spec's `servers[0].url` base URL and pull date.  The source
paths live in the editable, self-healing registry `dev/scm_sources.json`.
Refresh it with the `docsupdate` trigger (`python dev/update_scm_docs.py`).
...
Helper flags:
- `python dev/update_scm_docs.py --check` — report drift/relocations/endpoint changes, write nothing
- `python dev/update_scm_docs.py --list-remote` — print the live spec tree on pan.dev
- `python dev/update_scm_docs.py --no-mirror` — curated guides only (skip mirroring every doc)
- `python dev/update_scm_docs.py --self-test` — offline tests for discovery + diff logic
...
After a successful run, the script automatically calls `dev/gen_api_index.py`
