...
Create a **local-user-group** object in the active SCM folder.
...
```text
configure
set local-user-group <name> [<type-or-field> <value>] [description <text>] [tag <name>]
```

## API

```
POST /config/identity/v1/local-user-groups
```

Resource notes: group of local user accounts
...
arc:global # set local-user-group MyObject ...
  ✓ Local-User-Group MyObject created (id: ...)
```

## Related commands

- `show local-user-group` — list local-user-group objects in the active folder
- `delete local-user-group <name>` — remove a local-user-group object
