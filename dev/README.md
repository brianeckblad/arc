...
docsupdate  →  python dev/docsupdate.py
                 ...
                 ├─► generate_api_index.py        → dev/API_INDEX.md
...
pre-commit hook (.git/hooks/pre-commit, installed by dev/install_hooks.sh)
                 ├─► generate_code_map.py  → dev/CODE_MAP.md  (when app/*.py staged)
                 └─► smoke_test.py --quiet
...
| **`generate_code_map.py`** | AST-parses every ≥300-line file under `app/` into a method → line-range map. | pre-commit hook; manual; freshness checked by smoke §11 | `dev/CODE_MAP.md` |
| **`smoke_test.py`** | Structural test suite, 11 sections (syntax, imports, registry, arg parser, config, formatter, banner, builtin/help sync, completion, theme+docs, code-map freshness). No network. `--only N,…`, `--file <path>`. | pre-commit hook; manual after any `app/` change | exit 0/1 |
...
```bash
python dev/docsupdate.py                 # full refresh (self-healing) + all generators
...
python dev/panosupdate.py && python dev/generate_panos_catalog.py   # PAN-OS only
python dev/generate_code_map.py          # after moving methods in a 300+ line file
bash dev/install_hooks.sh                # one-time hook install
```

## Outputs that live outside `dev/` (intentional — they ship with ARC)
