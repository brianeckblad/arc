...
Usage:
    python dev/gen_code_map.py           # writes dev/CODE_MAP.md
    python dev/gen_code_map.py --check   # exit 1 if dev/CODE_MAP.md is stale
    python dev/gen_code_map.py --print    # print to stdout, don't write
...
def main() -> int:
    ...
    if args.check:
        if not OUT_FILE.exists():
            print("dev/CODE_MAP.md is missing — run: python dev/gen_code_map.py", file=sys.stderr)
            return 1
        current = OUT_FILE.read_text(encoding="utf-8")
        if current != content:
            print("dev/CODE_MAP.md is STALE — run: python dev/gen_code_map.py", file=sys.stderr)
            return 1
