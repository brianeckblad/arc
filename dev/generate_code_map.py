#!/usr/bin/env python3
"""Generate dev/CODE_MAP.md — an accurate method→line-range map of large files.
...
Usage:
    python dev/generate_code_map.py           # writes dev/CODE_MAP.md
    python dev/generate_code_map.py --check   # exit 1 if dev/CODE_MAP.md is stale
    python dev/generate_code_map.py --print    # print to stdout, don't write
...
def main() -> int:
    ...
    parser.add_argument("--check", action="store_true",
                        help="Exit 1 if dev/CODE_MAP.md is stale (used by smoke test).")
    ...
    if args.check:
        if not OUT_FILE.exists():
            print("dev/CODE_MAP.md is missing — run: python dev/generate_code_map.py", file=sys.stderr)
            return 1
        current = OUT_FILE.read_text(encoding="utf-8")
        if current != content:
            print("dev/CODE_MAP.md is STALE — run: python dev/generate_code_map.py", file=sys.stderr)
            return 1
        print("dev/CODE_MAP.md is current.")
        return 0
