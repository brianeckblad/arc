...
def _load_existing_states() -> dict[str, object]:
    """Preserve persisted states from files that have ``_carry: true``.

    A file with ``"_carry": false`` (or without a ``_carry`` key, which defaults
    to ``false`` for generator-owned files) is treated as always-fresh: existing
    values are ignored and the flag will be regenerated from scratch.

    ``local.json`` is exempt from generation entirely and is never read here
    (its values are loaded at runtime by the features loader).

    ``_carry: true`` must be explicitly set in the file to opt into preservation.
    This prevents the "silent revert" bug where a docsupdate wipes manual edits
    from generator-owned files.
    """
    ...
    for path in paths:
        if path.name == "local.json":
            continue  # never managed by the generator
        try:
        ...
        if not isinstance(raw, dict):
            continue
        # Only carry values from files that explicitly opt in with _carry: true.
        carry = raw.get("_carry", False)
        if not carry:
            continue
...
def _file_readme(stem: str) -> list[str]:
    """Return the _README block as a list of short lines (rendered as a JSON array).

    Using a list keeps each sentence on its own line in the JSON file so the
    comment is readable in any editor without horizontal scrolling.
    """
    kind_line = {
        "panos-ops": "PAN-OS operational command families plus curated live-device commands (ping, logs, system state — SSH/--remote).",
        "panos-config": "PAN-OS device-local config tree — BREAK-GLASS recovery only. config_recovery stays on; enable others only when SCM is unreachable.",
    ...
    return [
        kind_line,
        "Values: true | \"dev\" | false.",
        "_default: false = absent flags are off.",
        "_carry: false = generator always regenerates fresh (default);",
        "_carry: true = generator preserves existing true/dev values across rebuilds.",
        "Set _carry: true in this file to protect manual edits from docsupdate.",
        "Or put overrides in settings/features/local.json (never touched by generators).",
        "Regenerated by app/scripts/generate_feature_flags.py.",
...
def build_feature_files() -> dict[str, dict[str, object]]:
    ...
    def _file_map(stem: str) -> dict[str, object]:
        if stem not in files:
            # Check if the existing file has _carry: true — if so, carry that forward.
            existing_path = FEATURES_DIR / f"{stem}.json"
            carry = False
            if existing_path.exists():
                try:
                    carry = bool(json.loads(existing_path.read_text(encoding="utf-8")).get("_carry", False))
                except Exception:
                    carry = False
            files[stem] = {"_README": _file_readme(stem), "_default": False, "_carry": carry}
        return files[stem]
    ...
    # Final pass: for files with _carry: true, write back any user-added flags
    # that weren't in the catalog or explicit set (flags the generator doesn't
    # know about but the user manually added with a non-false value).
    carry_files: dict[str, dict[str, object]] = {}
    if FEATURES_DIR.is_dir():
        for path in FEATURES_DIR.glob("*.json"):
            ...
            try:
                ...
                continue
            if not isinstance(raw, dict) or not raw.get("_carry", False):
                continue
            stem = path.stem
            carry_files[stem] = raw

    all_emitted = emitted | (explicit - emitted)
    for stem, raw in carry_files.items():
        target = _file_map(stem)
        for key, val in raw.items():
            if key.startswith("_") or key in all_emitted:
                continue  # skip meta keys and already-generated flags
            if val is not False and val is not None:
                target[key] = val  # preserve user-added non-false flag
...
def main() -> int:
    ...
    if check_only:
        ...
        for name, rendered in expected.items():
            ...
            if not path.exists() or path.read_text(encoding="utf-8") != rendered:
                drift.append(name)
        if drift:
            print("settings/features/ is STALE — run: python app/scripts/generate_feature_flags.py")
    ...
    augment_feature_labels()
    return 0


def augment_feature_labels() -> None:
    """Add newly-discovered areas to settings/feature-labels.json, edit-safe.

    Keeps the human-readable naming layer (used by both the CLI and the GUI) in
    sync as new specs introduce new command categories.  Existing entries are
    NEVER overwritten — only missing areas are added (with a best-guess name),
    so operator edits survive every regeneration.
    """
    ...
    try:
        ...
        print(f"feature-labels: skipped (registry unavailable: {exc})")
        return

    categories = sorted({
        cd.category for cd in COMMANDS.values()
        if getattr(cd, "feature_flag", "") and cd.category
    })

    try:
        raw = json.loads(FEATURE_LABELS_JSON.read_text(encoding="utf-8")) \
            if FEATURE_LABELS_JSON.exists() else {}
    except (json.JSONDecodeError, OSError):
        raw = {}
    if not isinstance(raw, dict):
        raw = {}
    ...
    for cat in categories:
        if cat in areas:
            continue  # preserve any existing (possibly human-edited) name
        # Best guess: reuse a known label, else title-case the key.
        guess = _CATEGORY_LABELS.get(cat) or cat.replace("-", " ").replace("_", " ").title()
        areas[cat] = guess
        added.append(cat)

    if not added:
        print("feature-labels: current (no new areas)")
        return

    raw["areas"] = areas
    raw.setdefault("acronyms", {})
    raw.setdefault("_README", [
        "Human-readable names for the ARC feature editor (GUI + CLI).",
        "User-editable: rename freely. Missing keys fall back to a title-cased key.",
        "Auto-augmented by app/scripts/generate_feature_flags.py (edit-safe).",
    ])
    try:
        FEATURE_LABELS_JSON.write_text(json.dumps(raw, indent=2) + "\n", encoding="utf-8")
        print(f"feature-labels: added {len(added)} new area(s): {', '.join(added)}")
    except OSError as exc:
        print(f"feature-labels: could not write ({exc})")
