...
``app/scripts/panosupdate.py``) plus the hand-maintained knobs in
``app/scripts/panos-curation.json`` and emits ``PANOS_CATALOG`` — one entry per command
...
Usage::
    ...
    python app/scripts/generate_panos_catalog.py            # regenerate + report
    python app/scripts/generate_panos_catalog.py --check    # drift check, write nothing
"""
...
def build_catalog() -> tuple[list[dict], dict]:
    ...
    for page in pages:
        ...
        if not mirror.exists():
            print(f"  ⚠ mirror missing for {page['key']} — run app/scripts/panosupdate.py first; skipping")
...
def render(catalog: list[dict]) -> str:
    lines = [
        ...
        "",
        "Generated by ``app/scripts/generate_panos_catalog.py`` from the PAN-OS CLI mirrors",
        "under ``docs/panos-cli/`` (pulled by ``app/scripts/panosupdate.py``).  Each entry is",
        "one command stem; ``app/commands/panos_generated.py`` turns entries into",
        "feature-gated CommandDefs.",
        "",
        "Regenerate with:",
        "    python app/scripts/panosupdate.py               # refresh mirrors (network)",
        "    python app/scripts/generate_panos_catalog.py    # rebuild this file",
        '"""',
        "",
        "from __future__ import annotations",
        "",
    ...
    if check_only:
        if rendered != current:
            print("\napp/commands/panos_catalog.py is STALE — mirrors or curation "
                  "changed. Run: python app/scripts/generate_panos_catalog.py")
            return 1
