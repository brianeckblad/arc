...
Reads every .yaml in docs/scm-api/specs/ and produces app/scripts/API_INDEX.md —
a one-line-per-resource table that replaces reading individual spec files
(5,000+ lines) when looking up an endpoint.

Usage:
    python app/scripts/generate_api_index.py          # writes app/scripts/API_INDEX.md
    python app/scripts/generate_api_index.py --check  # prints without writing
    python app/scripts/generate_api_index.py --cat network  # one spec only
...
OUT_FILE   = ROOT / "app" / "scripts" / "API_INDEX.md"
COMMAND_DOCS_DIR = ROOT / "docs" / "commands"

# ---------------------------------------------------------------------------
# Known ARC CLI commands (from app.commands.registry.COMMANDS).
# Refreshed by running: python app/scripts/generate_api_index.py
...
def _build_index(specs: list[dict]) -> str:
    ...
    lines: list[str] = [
        ...
        "<!--",
        "  Generated by app/scripts/generate_api_index.py — do NOT edit by hand.",
        "  Refresh with: python app/scripts/generate_api_index.py",
        "",
